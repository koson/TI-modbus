...
static const uint8_t sha512_224Oid[] = {0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x05};
...
error_t sha512_224Compute(const void *data, size_t length, uint8_t *digest)
{
   ...
   Sha512_224Context *context = cryptoAllocMem(sizeof(Sha512_224Context));
...
void sha512_224Final(Sha512_224Context *context, uint8_t *digest)
{
   ...
   if(digest != NULL)
      cryptoMemcpy(digest, context->digest, SHA512_224_DIGEST_SIZE);
}
