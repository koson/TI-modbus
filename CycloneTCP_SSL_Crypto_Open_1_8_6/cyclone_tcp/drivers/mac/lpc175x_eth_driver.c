...
error_t lpc175xEthUpdateMacAddrFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_DEBUG("Updating LPC175x hash table...\r\n");
...
void lpc175xEthWritePhyReg(uint8_t phyAddr, uint8_t regAddr, uint16_t data)
{
   ...
   lpc175xEthWriteSmi(SMI_WRITE, 2);
   //Write PHY address
...
uint32_t lpc175xEthReadSmi(uint_t length)
{
   ...
   while(length--)
   {
      ...
      //Check MDIO state
      if(LPC175X_ETH_MDIO_GPIO->FIOPIN & LPC175X_ETH_MDIO_MASK)
         data |= 0x00000001;
   }
