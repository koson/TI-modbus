...
#include "core/net.h"
#include "drivers/mac/sam9263_eth_driver.h"
#include "debug.h"
...
void sam9263EthEventHandler(NetInterface *interface)
{
   ...
   if(rsr & (AT91C_EMAC_OVR | AT91C_EMAC_REC | AT91C_EMAC_BNA))
   {
      ...
      do
      {
         ...
         error = sam9263EthReceivePacket(interface);
...
error_t sam9263EthReceivePacket(NetInterface *interface)
{
   ...
   for(i = 0; i < SAM9263_ETH_RX_BUFFER_COUNT; i++)
   {
      ...
      if(j >= SAM9263_ETH_RX_BUFFER_COUNT)
