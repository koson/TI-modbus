...
#include "lpc18xx.h"
#include "core/net.h"
#include "drivers/mac/lpc18xx_eth_driver.h"
#include "debug.h"
...
static uint8_t txBuffer[LPC18XX_ETH_TX_BUFFER_COUNT][LPC18XX_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
static uint8_t rxBuffer[LPC18XX_ETH_RX_BUFFER_COUNT][LPC18XX_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
static Lpc18xxTxDmaDesc txDmaDesc[LPC18XX_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
static Lpc18xxRxDmaDesc rxDmaDesc[LPC18XX_ETH_RX_BUFFER_COUNT];
...
static Lpc18xxTxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Lpc18xxRxDmaDesc *rxCurDmaDesc;


/**
 * @brief LPC18xx Ethernet MAC driver
 **/
...
/**
 * @brief LPC18xx Ethernet MAC initialization
...
error_t lpc18xxEthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing LPC18xx Ethernet MAC...\r\n");
   ...
   lpc18xxEthInitDmaDesc(interface);
   ...
   NVIC_SetPriority(ETHERNET_IRQn, NVIC_EncodePriority(LPC18XX_ETH_IRQ_PRIORITY_GROUPING,
      LPC18XX_ETH_IRQ_GROUP_PRIORITY, LPC18XX_ETH_IRQ_SUB_PRIORITY));
...
void lpc18xxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < LPC18XX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < LPC18XX_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ETH_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ETH_RDES1_RCH | (LPC18XX_ETH_RX_BUFFER_SIZE & ETH_RDES1_RBS1);
...
/**
 * @brief LPC18xx Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief LPC18xx Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void lpc18xxEthEventHandler(NetInterface *interface)
{
   ...
   if(LPC_ETHERNET->DMA_STAT & (ETHERNET_DMA_STAT_RI_Msk | ETHERNET_DMA_STAT_OVF_Msk))
   {
      ...
      do
      {
         ...
         error = lpc18xxEthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   if(length > LPC18XX_ETH_TX_BUFFER_SIZE)
   {
...
error_t lpc18xxEthReceivePacket(NetInterface *interface)
{
   ...
   if(!(rxCurDmaDesc->rdes0 & ETH_RDES0_OWN))
   {
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Lpc18xxRxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
...
error_t lpc18xxEthUpdateMacAddrFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_DEBUG("Updating LPC18xx hash table...\r\n");
   ...
   for(i = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = lpc18xxEthCalcCrc(&entry->addr, sizeof(MacAddr));
