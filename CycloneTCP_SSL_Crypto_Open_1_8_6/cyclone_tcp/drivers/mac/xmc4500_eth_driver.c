...
#include "xmc4500.h"
#include "core/net.h"
#include "drivers/mac/xmc4500_eth_driver.h"
#include "debug.h"
...
error_t xmc4500EthInit(NetInterface *interface)
{
   ...
   NVIC_SetPriorityGrouping(XMC4500_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ETH0_0_IRQn, NVIC_EncodePriority(XMC4500_ETH_IRQ_PRIORITY_GROUPING,
...
void xmc4500EthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < XMC4500_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < XMC4500_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ETH_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ETH_RDES1_RCH | (XMC4500_ETH_RX_BUFFER_SIZE & ETH_RDES1_RBS1);
...
void xmc4500EthEventHandler(NetInterface *interface)
{
   ...
   if(ETH0->STATUS & ETH_STATUS_RI_Msk)
   {
      ...
      do
      {
         ...
         error = xmc4500EthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   if(length > XMC4500_ETH_TX_BUFFER_SIZE)
   {
