...
error_t lan8742Init(NetInterface *interface)
{
   ...
   lan8742WritePhyReg(interface, LAN8742_PHY_REG_ANAR, ANAR_100BTX_FD |
      ANAR_100BTX | ANAR_10BT_FD | ANAR_10BT | ANAR_SELECTOR0);

   //Enable auto-negotiation
   lan8742WritePhyReg(interface, LAN8742_PHY_REG_BMCR, BMCR_AN_EN);

   //Dump PHY registers for debugging purpose
   lan8742DumpPhyReg(interface);

   //The PHY will generate interrupts when link status changes are detected
   lan8742WritePhyReg(interface, LAN8742_PHY_REG_IMR,
      IMR_AN_COMPLETE | IMR_LINK_DOWN);
