...
#define APP_MAC_ADDR "00-AB-CD-EF-18-30"
...
#define APP_IPV6_LINK_LOCAL_ADDR "fe80::1830"
#define APP_IPV6_PREFIX "2001:db8::"
#define APP_IPV6_PREFIX_LENGTH 64
#define APP_IPV6_GLOBAL_ADDR "2001:db8::1830"
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, uint16_t packetId)
{
   ...
   if(!strcmp(topic, "board/leds/1"))
   {
      if(length == 6 && !strncasecmp((char_t *) message, "toggle", 6))
         LPC_GPIO_PORT->NOT[LED2_PORT] = LED2_MASK;
   ...
   {
      if(length == 6 && !strncasecmp((char_t *) message, "toggle", 6))
         LPC_GPIO_PORT->NOT[LED3_PORT] = LED3_MASK;
      else if(length == 2 && !strncasecmp((char_t *) message, "on", 2))
         LPC_GPIO_PORT->CLR[LED3_PORT] = LED3_MASK;
      else
         LPC_GPIO_PORT->SET[LED3_PORT] = LED2_MASK;;
   }
}
   ...
   while(1)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            buttonState = !(LPC_GPIO_PORT->PIN[SW2_PORT] & SW2_MASK);
...
int_t main(void)
{
   ...
   netSetDriver(interface, &lpc18xxEthDriver);
