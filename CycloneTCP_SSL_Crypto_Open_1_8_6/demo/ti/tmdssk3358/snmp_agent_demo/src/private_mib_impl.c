   ...
   size_t oidLen, const MibVariant *value, size_t valueLen, bool_t commit)
{
   ...
   if(!strcmp(object->name, "ledState"))
   {
      ...
      if(commit)
      {
         ...
         {
            if(entry->ledState)
               GPIOPinWrite(SOC_GPIO_1_REGS, 4, GPIO_PIN_HIGH);
            else
               GPIOPinWrite(SOC_GPIO_1_REGS, 4, GPIO_PIN_LOW);
         }
         else if(index == 2)
         {
            if(entry->ledState)
         ...
         {
            if(entry->ledState)
               GPIOPinWrite(SOC_GPIO_1_REGS, 7, GPIO_PIN_HIGH);
            else
               GPIOPinWrite(SOC_GPIO_1_REGS, 7, GPIO_PIN_LOW);
         }
      }
   }
   //Unknown object?
