...
#define UART0_INPUT_CLK 48000000
...
void debugInit(uint32_t baudrate)
{
   ...
   //Configure the system clocks for UART0 instance
   UART0ModuleClkConfig();

   //Configure UART pin multiplexing
   UARTPinMuxSetup(0);
   ...
   //Set FIFO settings
   temp = UART_FIFO_CONFIG(UART_TRIG_LVL_GRANULARITY_4,
      UART_TRIG_LVL_GRANULARITY_1, UART_FCR_TX_TRIG_LVL_56,
      1, 1, 1, UART_DMA_EN_PATH_SCR, UART_DMA_MODE_0_ENABLE);

   //Configure UART FIFO
   UARTFIFOConfig(SOC_UART_0_REGS, temp);

   //Compte the divisor value
   temp = UARTDivisorValCompute(UART0_INPUT_CLK, baudrate,
      UART16x_OPER_MODE, UART_MIR_OVERSAMPLING_RATE_42);

   //Program the divisor latch
   UARTDivisorLatchWrite(SOC_UART_0_REGS, temp);
   ...
   UARTLineCharacConfig(SOC_UART_0_REGS, UART_FRAME_WORD_LENGTH_8 |
      UART_FRAME_NUM_STB_1, UART_PARITY_NONE);

   //Disable write access to the divisor latch
   UARTDivisorLatchDisable(SOC_UART_0_REGS);

   //Disable break control
   UARTBreakCtl(SOC_UART_0_REGS, UART_BREAK_COND_DISABLE);

   //Switch to UART 16x operating mode
