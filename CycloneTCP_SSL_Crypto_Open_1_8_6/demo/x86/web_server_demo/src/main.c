...
__declspec(dllimport) uint32_t __stdcall WaitForSingleObject(
   void *hHandle, uint32_t dwMilliseconds);
...
int_t main(void)
{
   ...
   //Create user task
   task = osCreateTask("User Task", userTask, NULL, 800, OS_TASK_PRIORITY_NORMAL);
   ...
   WaitForSingleObject(task, INFINITE_DELAY);
   ...
   const char_t *param)
{
   ...
   {
      strcpy(connection->buffer, "Simulator");
   }
