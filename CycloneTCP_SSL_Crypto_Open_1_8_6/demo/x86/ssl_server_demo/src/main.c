...
error_t tlsPskCallback(TlsContext *context, const char_t *pskIdentity);
 ...
 * @param[in] pskIdentity NULL-terminated string that contains the
 *   PSK identity of the client
 * @return Error code
 **/
...
error_t tlsPskCallback(TlsContext *context, const char_t *pskIdentity)
{
   ...
   //Check PSK identity
   if(!strcmp(pskIdentity, APP_CLIENT1_PSK_IDENTITY))
   {
      //Set the pre-shared key to be used
      error = tlsSetPsk(context, client1Psk, sizeof(client1Psk));
   }
   else if(!strcmp(pskIdentity, APP_CLIENT2_PSK_IDENTITY))
   {
      //Set the pre-shared key to be used
