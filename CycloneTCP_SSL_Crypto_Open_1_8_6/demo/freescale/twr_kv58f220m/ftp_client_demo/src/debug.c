...
void debugInit(uint32_t baudrate)
{
   ...
   //Clock 4 output divider value (bus clock)
   div = (SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV4_MASK) >> SIM_CLKDIV1_OUTDIV4_SHIFT;
   //UART0 module clock frequency
   moduleClockFreq = SystemCoreClock / (div + 1);

   //Compute the UART baudrate prescaler
   div = ((4 * moduleClockFreq) + (baudrate / 2)) / baudrate;
   ...
   //Configure PTB0 (UART0_RX) and PTB1 (UART0_TX)
   PORTB->PCR[0] = PORT_PCR_MUX(7);
   PORTB->PCR[1] = PORT_PCR_MUX(7);
