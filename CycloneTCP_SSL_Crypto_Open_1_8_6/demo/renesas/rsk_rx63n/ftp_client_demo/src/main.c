...
void lcdPutChar(char_t c)
{
   ...
   else if(lcdLine < 2 && lcdColumn < 8)
   {
      buffer[0] = c;
      buffer[1] = '\0';

      //Display current character
      Display_LCD(lcdLine * 16 + lcdColumn, buffer);
...
int_t main(void)
{
   ...
   netSetPhyDriver(interface, &lan8700PhyDriver);
...
void vApplicationSetupTimerInterrupt(void)
{
   ...
   CMT1.CMCOR = ((PCLK_HZ / configTICK_RATE_HZ) - 1) / 8;
