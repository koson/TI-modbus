   ...
   size_t oidLen, const MibVariant *value, size_t valueLen, bool_t commit)
{
   ...
   if(!strcmp(object->name, "ledState"))
   {
      ...
      if(commit)
      {
         ...
         {
            if(entry->ledState)
               LED2_IOPORT->PCNTR3 = LED2_MASK << 16;
            else
         ...
         {
            if(entry->ledState)
               LED3_IOPORT->PCNTR3 = LED3_MASK << 16;
            else
