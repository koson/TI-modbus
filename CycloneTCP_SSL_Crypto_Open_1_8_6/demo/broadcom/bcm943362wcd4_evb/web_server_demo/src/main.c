...
#define APP_WIFI_AP_KEY "\x11\x22\x33\x44\x55"
...
int_t main(void)
{
   ...
   task = osCreateTask("Init", initTask, NULL, 700, OS_TASK_PRIORITY_NORMAL);
...
void initTask(void *param)
{
   ...
   else if(APP_WIFI_AP_SECURITY == WICED_SECURITY_WEP_PSK)
   {
      ...
      if(strlen(APP_WIFI_AP_KEY) == 5)
      {
         ...
         for(i = 0; i < 4; i++)
         {
            ...
            wepKey->length = 5;
            //Copy WEP key
            memcpy(wepKey->data, APP_WIFI_AP_KEY, wepKey->length);
         }
      ...
      //WEP-104 key?
      else if(strlen(APP_WIFI_AP_KEY) == 13)
      {
         //Format WEP keys
         for(i = 0; i < 4; i++)
         {
   ...
   else if(APP_WIFI_AP_SECURITY == WICED_SECURITY_WPA2_MIXED_PSK)
   {
      ...
      keyLength = strlen(APP_WIFI_AP_KEY);

      //Start the access point
      ret = wwd_wifi_start_ap(&ssid, WICED_SECURITY_WPA2_MIXED_PSK,
         (uint8_t *) APP_WIFI_AP_KEY, keyLength, APP_WIFI_AP_CHANNEL);
   }
   ...
   else if(APP_WIFI_STA_SECURITY == WICED_SECURITY_WEP_PSK)
   {
      ...
      if(strlen(APP_WIFI_STA_KEY) == 5)
      {
         ...
         for(i = 0; i < 4; i++)
         {
            ...
            wepKey->length = 5;
            //Copy WEP key
            memcpy(wepKey->data, APP_WIFI_STA_KEY, wepKey->length);
         }
      ...
      //WEP-104 key?
      else if(strlen(APP_WIFI_STA_KEY) == 13)
      {
         //Format WEP keys
         for(i = 0; i < 4; i++)
         {
   ...
   {
      //Get the length of the key
      keyLength = strlen(APP_WIFI_STA_KEY);

      //Join the desired Wi-Fi network
      ret = wwd_wifi_join(&ssid, WICED_SECURITY_WPA2_MIXED_PSK,
         (uint8_t *) APP_WIFI_STA_KEY, keyLength, NULL);
   }
